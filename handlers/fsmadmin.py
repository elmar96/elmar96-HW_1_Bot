...
async def complete_delete_user(call: types.CallbackQuery):
    await bot_db.sql_delete_user(call.data.replace("delete ", ""))
    await call.answer(text=f'{call.data.replace("delete ", "")} deleted',
                      show_alert=True
                      )


async def delete_data_user(message: types.Message):
    selected_data_user = await bot_db.sql_casual_select_user()
    for result in selected_data_user:
        await bot.send_photo(
            message.chat.id,
            result[0],
            caption=f'user_name: {result[1]}\n first_name {result[2]}',
...
def register_handler_admin(dp: Dispatcher):
    ...
    dp.register_message_handler(delete_data_user, commands=['delete_user'])
